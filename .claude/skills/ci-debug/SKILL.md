...
`.github/workflows/update-feed.yml` runs at 06:23/18:23 UTC and commits `feed.json` only when it changed. Use the GitHub MCP tools (`mcp__github__actions_list`, `actions_get`, `get_job_logs`) — there is no `gh` CLI here.
...
- **`✗ Chunk N failed in batch` / `Batch submit failed`** → translation API errors (rate limit, overload, batch trouble). One-offs self-heal via the direct-call fallback, the retry pass, and the next 12-hour run. Persistent 401s = key problem (see above).
...
- **No runs at all around a scheduled slot** → GitHub drops scheduled slots under load (that's why it runs twice daily rather than once) and **disables schedules entirely after ~60 days without repo activity**. If the last run is old and the repo has been quiet, the user needs to re-enable the workflow in the Actions tab; suggest a manual `workflow_dispatch` (`actions_run_trigger`) to confirm it still works.
...
- The commit step's `git diff --cached --quiet && exit 0` no-change guard must survive any edit, or the repo gets a junk commit twice a day.
